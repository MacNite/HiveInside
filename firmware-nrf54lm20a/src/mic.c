/* mic.c — see mic.h.
 *
 * Ported from the ESP32-C6 prototype's mic.cpp, minus its whole software
 * CIC decimator: the nRF54's PDM peripheral filters PDM to 16-bit PCM in
 * hardware, so this module only gathers PCM blocks through Zephyr's dmic
 * API and runs the unchanged dBFS + FFT band analysis.
 *
 * The PDM clock only runs between DMIC_TRIGGER_START and _STOP, so the
 * microphone (powered from nPM1300 LDO1 — see power.c) draws its idle
 * current outside the ~0.7 s capture.
 *
 * The PDM controller is resolved by compatible, not by node label: any
 * enabled `nordic,nrf-pdm` node is used (the Sense board wires it to the
 * on-board MSM261DGT006). If the devicetree has none enabled, the module
 * compiles to a stub and the beacon simply reports the mic group absent.
 */
#include "mic.h"
...
static float band_energy_dbfs(const float *mag, float lo_hz, float hi_hz,
			      float norm)
{
	/* Parseval sum (no /count) so the value reflects real in-band
	 * acoustic energy and is comparable between bands of different
	 * widths — same convention as the prototype and HiveScale. */
...
void mic_read(struct measurement *m)
{
	...
	/* Stop the PDM clock between captures — the microphone consumes
	 * power whenever it is clocked. */
	...
	printk("[MIC] rms=%.1f peak=%.1f dBFS | sub=%.1f hum=%.1f pipe=%.1f stress=%.1f hi=%.1f (%u frames)\n",
	       (double)m->mic_rms_dbfs, (double)m->mic_peak_dbfs,
	       (double)m->mic_band_sub_bass_dbfs, (double)m->mic_band_hum_dbfs,
	       (double)m->mic_band_piping_dbfs, (double)m->mic_band_stress_dbfs,
	       (double)m->mic_band_high_dbfs, (unsigned)count);
}
...
void mic_read(struct measurement *m)
{
	m->mic_ok = false;
#if ENABLE_MIC
	/* Devicetree has no enabled PDM controller: see README ("Enabling
	 * the microphone") for the board overlay that turns it on. */
