...
— see the top of this document. Build it in a west workspace that has the XIAO
nRF54LM20A board definition available: the repository's `west.yml` pins an
upstream Zephyr revision carrying it, and an nRF Connect SDK workspace needs the
...
Zephyr release up to v4.4.2 contains `boards/seeed/xiao_nrf54lm20a/`, which is
why this repository's `west.yml` pins a `main` commit rather than a release.
...
* **Build against upstream Zephyr** — `west init -m <this repo>` gives you a
  workspace at the pinned revision. This is the path the rest of this document
  describes and needs no extra setup.
